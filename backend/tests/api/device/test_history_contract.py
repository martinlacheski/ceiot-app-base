...
def test_history_routes_and_bounded_query_parameters():
    ...
    assert {prefix, f"{prefix}/{{serial}}/operations"} <= paths.keys()
    # S6: the fixed-column sensor-readings endpoint is gone; telemetry (JSONB)
    # is the only per-serial environmental history endpoint now.
    assert f"{prefix}/{{serial}}/sensor-readings" not in paths.keys()
    for route in (prefix, f"{prefix}/{{serial}}/operations"):
...
def test_history_detail_dates_are_validated_and_forwarded(client, token, monkeypatch):
    ...
    monkeypatch.setattr(DeviceHistoryService, "list_operations", fake_operations)
    headers = {"Authorization": f"Bearer {token}"}
    for suffix, key in (("operations", "operations"),):
