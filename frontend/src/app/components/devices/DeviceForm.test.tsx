...
describe("DeviceForm", () => {
  it("requires sensors for Ambiental and submits repeated models", async () => {
    ...
    expect(table.compareDocumentPosition(screen.getByText("Agregá al menos un sensor")) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    ...
    expect(screen.queryByText("Modelo del sensor 1")).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Quitar sensor" })).toHaveAttribute("title", "Quitar sensor");
    expect(screen.getByRole("option", { name: "DHT22" })).toBeInTheDocument();
