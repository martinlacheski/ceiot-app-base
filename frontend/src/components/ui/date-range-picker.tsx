...
import { type FC, useState, useRef, type JSX, useEffect } from "react";
...
const getPreviousCalendarYearDate = (date: Date): Date => {
  ...
  const lastDayOfMonth = new Date(previousYear, date.getMonth() + 1, 0).getDate();
...
export const DateRangePicker: FC<DateRangePickerProps> = ({
  ...
  const trigger = (
    <Button
      size="lg"
      variant="outline"
      className={cn(mobileLayout && "min-h-11 w-full justify-between", triggerClassName)}
    >
        ...
        >
              ...
              >
                ...
                >
                  ...
                  >
                    <DateBoundaryPicker
                      ...
                      onSelect={(date) => {
                        setRange({
                          from: date,
                          to: range.to == null || date > range.to ? date : range.to,
                        });
                    ...
                    <DateBoundaryPicker
                      ...
                      value={range.to}
                      onSelect={(date) => {
                        setRange({ from: date < range.from! ? date : range.from!, to: date });
                        setCalendarMonth(date);
                      }}
                    />
                  </div>
                  ...
                  {isSmallScreen && !isWideShortLandscape && (
                    <div className="min-w-0">
                      <Label className="sr-only mb-1 text-xs">
                        Período
                      </Label>
                      <Select
                      ...
                      >
                        <SelectTrigger className={cn("mx-auto mb-1 w-[180px]", mobileLayout && "min-h-11 w-full")}>
                          <SelectValue placeholder="Seleccionar..." />
                        </SelectTrigger>
                        <SelectContent className={cn(isMobileDialog && "z-[202]")}>
                          <SelectGroup>
                            {PRESETS.map((preset) => (
          ...
          {(!isSmallScreen || isWideShortLandscape) && (
            <div
              ...
              className={cn(
                "flex flex-col items-end gap-1 pr-2 pl-6 pb-6",
                isWideShortLandscape && "shrink-0 px-1 pb-0",
              )}
            >
              <div
                className={cn(
                  "flex w-full flex-col items-end gap-1 pr-2 pl-6 pb-6",
                  isWideShortLandscape && "gap-0 px-0 pb-0",
                )}
              >
                {PRESETS.map((preset) => (
                  <PresetButton
    ...
    <Popover modal open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent
        ...
        className={cn(
          "w-auto",
          mobileLayout &&
            "flex max-h-[var(--radix-popover-content-available-height)] w-[calc(100vw-2rem)] max-w-md flex-col overflow-hidden p-0 sm:w-auto sm:max-w-none",
        )}
      >
        {pickerContent}
      </PopoverContent>
