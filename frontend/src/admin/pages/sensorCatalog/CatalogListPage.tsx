...
import { Ban, Pencil, Plus, RotateCw } from "lucide-react";
...
export function CatalogListPage({ kind }: { kind: CatalogKind }) {
  ...
  const columns = useMemo<ColumnDef<Item>[]>(() => [
    ...
    { id: "actions", header: () => <CenteredHeader>Acciones</CenteredHeader>, cell: ({ row }) => <div className="flex justify-center gap-2">{writable && <><Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="size-8" asChild><Link to={`${path}/edit/${row.original.id}`} aria-label={`Editar ${sensors ? "sensor" : "variable"}`}><Pencil className="size-4" /></Link></Button></TooltipTrigger><TooltipContent>Editar</TooltipContent></Tooltip><Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="size-8" aria-label={`${row.original.isActive ? "Desactivar" : "Activar"} ${sensors ? "sensor" : "variable"}`} onClick={() => row.original.isActive ? deactivate(row.original) : activate(row.original)}>{row.original.isActive ? <Ban className="size-4" /> : <RotateCw className="size-4" />}</Button></TooltipTrigger><TooltipContent>{row.original.isActive ? "Desactivar" : "Activar"}</TooltipContent></Tooltip></>}</div> },
